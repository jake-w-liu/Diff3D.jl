...
- Done: add OpenEXR scanline and tiled (ONE_LEVEL + MIPMAP level 0) loading via
  ...
  RGB for scanline and tiled (ONE_LEVEL/MIPMAP level 0) images, with clear errors for deep,
  multi-part, RIPMAP tiles, subsampled, pLinear-B44, and DWA
...
- Added BufferGeometry draw ranges via `set_draw_range!`, `get_draw_range`,
  ...
  `WebGLRenderer` internals remain documented deviations.
- Added a standalone partial port for `webgl_buffergeometry_selective_draw` via
  `examples/webgl_buffergeometry_selective_draw.jl`, preserving the upstream
  `100 x 200` line-field scale plus per-vertex `:vertColor` and `:visible`
...
- Added a standalone partial port for `webgl_buffergeometry_attributes_none` via
  ...
  into explicit non-indexed `BufferGeometry` attributes. Upstream no-attribute
  `RawShaderMaterial`, exact GLSL hashing, `Stats`, and the imperative animation
  loop remain documented deviations.
- Added a standalone partial port for `webgl_buffergeometry_attributes_integer`
  via `examples/webgl_buffergeometry_attributes_integer.jl`, preserving the
  upstream 10,000-triangle scale, UVs, and per-vertex `Int16` `:textureIndex`
...
- Added a standalone partial port for `webgl_buffergeometry_glbufferattribute`
  via `examples/webgl_buffergeometry_glbufferattribute.jl`, preserving the
  upstream 300,000-particle scale, alternate `:position2` buffer data, raw
  `UInt8` `:colorBytes`, normalized browser vertex colors, and a 10,000-point
  draw-range snapshot. Upstream user-created `GLBufferAttribute` VBO handles,
  timed buffer swapping, random per-frame draw counts, `Stats`, and the
  imperative animation loop remain documented deviations.
- Added a standalone partial port for `webgl_buffergeometry_points_interleaved`
...
- Added a standalone partial port for `webgl_buffergeometry_instancing_interleaved`
  ...
  remain documented deviations.
- Added a standalone partial port for `webgl_lights_hemisphere` via
  `examples/webgl_lights_hemisphere.jl`, preserving the upstream hemisphere
  light HSL colors/intensity, directional light color/position/intensity, helper
  objects, 10,000-unit ground plane, fog, sky dome gradient, and animated
  flamingo subject. Upstream `Flamingo.glb`, `AnimationMixer`, custom
  `ShaderMaterial`, GUI controls, `Stats`, and `WebGLRenderer` shadow-map
  internals remain documented deviations.
- Added a standalone partial port for `webgl_lights_spotlight` via
  `examples/webgl_lights_spotlight.jl`, preserving the upstream camera, orbit
  target, hemisphere ambient, spotlight position path, intensity, angle,
  penumbra, decay, shadow bias, 10x10 receiving plane, helper object, and
  shadow-casting subject. Upstream `Lucy100k.ply`, `PLYLoader`,
  `SpotLight.map` texture projection, GUI controls, exact tone/shadow-map
  settings, and `OrbitControls` setup remain documented deviations.
- Added a standalone partial port for `webgl_lights_spotlights` via
  `examples/webgl_lights_spotlights.jl`, preserving the upstream floor, box,
  ambient light, three colored shadow-casting spotlights, initial positions,
  intensity, angle, penumbra, decay, distance, helpers, camera, and orbit target
  with deterministic animation tracks. Upstream random `TWEEN` motion,
  live helper updates, `OrbitControls`, and `WebGLRenderer` shadow-map runtime
  remain documented deviations.
- Added a standalone partial port for `webgl_lights_rectarealight` via
  `examples/webgl_lights_rectarealight.jl`, preserving the upstream three RGB
  `RectAreaLight`s, 4x10 light dimensions, intensity, positions, animated
  orientations, 2000-unit checker-rough floor, white torus knot, camera, and
...
- Added a standalone partial port for `webgl_camera_array` via
  ...
  documented deviations.
- Added a standalone partial port for `misc_animation_keys` via
  `examples/misc_animation_keys.jl`, mirroring the upstream single-box scene
...
- Added a standalone partial port for `misc_animation_groups` via
  ...
  remain documented deviations.
- Added a standalone partial port for `misc_boxselection` via
  `examples/misc_boxselection.jl`, using 200 deterministic Lambert boxes,
  upstream-style ambient/spot lighting, and a fixed projected selection
  rectangle that emissive-highlights selected boxes. Upstream pointer-driven
  `SelectionBox`/`SelectionHelper`, DOM selection overlay, random placement,
  and PCF shadow-map details remain documented deviations.
- Added a standalone partial port for `misc_controls_drag` via
  `examples/misc_controls_drag.jl`, using 200 deterministic Lambert boxes,
  upstream-style ambient/spot lighting, a fixed selected group with emissive
...
- Added a standalone partial port for `misc_controls_transform` via
  ...
  documented deviations.
- Added a standalone partial port for `misc_exporter_stl` via
  `examples/misc_exporter_stl.jl`, exporting a transformed box to binary STL with
  `save_stl_binary`, validating the binary payload size and face count, reloading
  it through `load_stl`, and rendering the round-tripped mesh in an upstream-style
...
- Added a standalone partial port for `misc_exporter_ply` via
  ...
  remain documented deviations.
- Added a standalone partial port for `misc_exporter_obj` via
  `examples/misc_exporter_obj.jl`, exporting deterministic triangle, cube,
  cylinder, and point-cloud records to an OBJ file, reloading the mesh records
  through `load_obj`, and rendering the round-tripped mesh alongside the source
  point records. Upstream `OBJExporter` GUI mode switching, Blob downloads, and
  WebGLRenderer resize/animation-loop internals remain documented deviations;
  point records are verified in the OBJ text because Diff3D.jl `load_obj` loads
  mesh faces only.
...
- Added a standalone partial port for `misc_controls_map` via
  `examples/misc_controls_map.jl`, using deterministic `InstancedMesh`
  building placement, `FogExp2`, Phong lighting, and local `OrbitControls`
  damping/distance/polar-angle settings. Upstream `MapControls`,
  `zoomToCursor`, `screenSpacePanning` GUI toggles, random placement, and exact
  browser map-control behavior remain documented deviations.
- Added a standalone partial port for `misc_controls_trackball` via
  `examples/misc_controls_trackball.jl`, using deterministic `InstancedMesh`
  cone placement, `FogExp2`, Phong lighting, local perspective
