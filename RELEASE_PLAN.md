...
| R4 | Verify the supported Julia versions on Linux, macOS, and Windows; exercise browser export on Chromium, Firefox, and WebKit. Preserve numerical, resource-lifetime, concurrency, and example checks. Record the tested architectures and browser/GPU environments. | In progress | Windows 1.10 installed-consumer rerun passed. Cube uploads repaired for NPOT and partial mip chains; matrices now pin Julia 1.10 and 1.13. Both Ubuntu browser failures have measured causes: Firefox needs an X display and WebKit honours the 10-bit `mediump` minimum. Exports now select `highp` where reported and Linux Firefox runs under Xvfb; the candidate Ubuntu run must confirm both |
