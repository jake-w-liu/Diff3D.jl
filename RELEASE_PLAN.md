...
| R4 | Verify the supported Julia versions on Linux, macOS, and Windows; exercise browser export on Chromium, Firefox, and WebKit. Preserve numerical, resource-lifetime, concurrency, and example checks. Record the tested architectures and browser/GPU environments. | In progress | Optimized shards passed on all three operating systems for Julia 1.10 and 1.13. Cube uploads repaired for NPOT and partial mip chains. The `highp` change closed the Ubuntu WebKit UV failure, confirmed on the candidate. The intermittent macOS Firefox `orbit_zoom_limits` failure is explained: the hosted macOS runners have no GPU and give Firefox Apple's software OpenGL renderer, which drops triangles that need clipping (reproduced natively through CGL and by three.js on the same scene). Firefox moves back to Linux, where Mesa llvmpipe rasterises the failing triangles exactly as an Apple M5 does; an occasional Firefox launch under Xvfb with no WebGL context (4 of 323 measured) is relaunched, and the harness refuses Apple's software renderer. The viewer's frame loop no longer dies on a frame that raises. The final candidate matrix must confirm the Linux Firefox arrangement |
