...
|---|---|---|---|
| R1 | Publish the compatibility contract: public names and documented signatures, supported backends and numeric/AD inputs, mutation and buffer ownership, concurrency, errors, extension points, and deprecation rules. Exercise the contract through public APIs. | Complete | Contract, 453-name inventory and public checks prepared; ForwardDiff 0.10.39 also passed 550 integration assertions. Candidate consumer checks passed on `3eb2300` (release-validation run 35947427671: all six installed-consumer jobs green) |
| R2 | Correct public documentation, examples, and installation instructions; execute consumer-facing examples; build strict docs with no stale API references. | Complete | Strict local docs build passed all 18 executable tutorials; documentation run 35947389008 on `3eb2300` passed, and every registered example was exercised by the green browser shards |
| R3 | Make the complete optimized test/allocation coverage reproducible without lowering budgets or dropping cases. Keep focused development checks and run the required release suite under the normal compiler. | Complete | Complete shard inventory and runner checks passed; all 36 optimized shards (3 OS × Julia 1.10/1.13 × 6) passed on `3eb2300` in run 35947427671, plus the 12 Ubuntu shards in standard CI run 35947389004 |
| R4 | Verify the supported Julia versions on Linux, macOS, and Windows; exercise browser export on Chromium, Firefox, and WebKit. Preserve numerical, resource-lifetime, concurrency, and example checks. Record the tested architectures and browser/GPU environments. | Complete | Full matrix green on `3eb2300` (run 35947427671): optimized shards on all three operating systems for Julia 1.10 and 1.13, and all 18 browser shards on Chromium, Firefox (Linux/Mesa llvmpipe) and WebKit, including the context-loss and GL-state checks. The macOS-Firefox blank-frame investigation is recorded in the evidence log (Apple software rasteriser; three.js reproduces it), and the chromium-4/6 starvation defect introduced by the robustness patch is fixed by the adaptive GL-error drain in `3eb2300`, verified by the green rerun |
| R5 | Add independent consumer acceptance workflows: clean package installation, asset import and render/export, and an inverse problem using public APIs. Check outputs/gradients against independent expectations and test documented unsupported-input failures. | Complete | Fresh Git installation passed 39 public assertions on Julia 1.12.7 and its export passed all three browsers; on `3eb2300` all six installed-consumer jobs (3 OS × Julia 1.10/1.13) and all three installed-export browser jobs passed in run 35947427671 |
| R6 | Run pinned, reproducible comparisons with three.js. Validate matching inputs and output accuracy before timing; publish raw timing/memory/size results, environment details, advantages, and limitations. Repair confirmed relevant bottlenecks and rerun affected comparisons. | Complete | The protocol's acceptance checks passed on the candidate: the pinned Linux/Chromium comparison re-ran on `3eb2300` (comparison/compare, run 35947427671) with all stages green, and the published [report](release/1.0/comparison.md) records 32 numerical records and 18 browser pairs with zero mismatches outside edge ties, the 3.47e-13 worst timed-gradient error, the 5.00e-11 recovery bound and the byte-identical `static-16` pixel hash; every table row derives from committed summaries that reproduce byte-for-byte from the committed raw archives. The only code change between that run and the candidate is the GL-error drain gate — timing logic that alters when errors are read, not what is drawn — and it is covered by the same green comparison. Supplementary note: the local macOS/Firefox pass remains pinned at `0dddaff`; its refresh is a local playwright procedure and is not re-run under the current prohibition on local playwright runs |
| R7 | Prepare versioned docs, tag/release and registry procedures, release notes, and migration guidance. Verify the workflow on a release candidate and confirm that the release source includes every accepted fix. | Complete | Version 1.0.0 metadata, changelog, versioned docs and publishing/migration procedures verified end to end on the candidate. `v1.0.0` is tagged at `7d8111ba8b48455e3e0df08f5248e981f2331def` (tree `d13c55d8df8df9d83dd61eaa9373471b659d4e40`) after release-validation run 35960703932 passed 66/66 plus standard CI and documentation runs on the same commit; the tag push deployed versioned docs (`/v1.0.0/` and `/stable/` both serve 200), the GitHub release carries the approved notes and comparison archives, and General registry PR 169302 — with the required breaking-change notes — is merged; `Pkg.add("Diff3D")` in a fresh environment resolves 1.0.0 whose manifest tree hash matches the tag |
| R8 | Review the final diff and contract independently of the implementation route; pass all required gates on the exact candidate revision; produce the reviewable version/tag/registration payload and final evidence report. | Complete | Source/contract review completed. The CI guard rejecting machine-specific paths anywhere in the published tree, including inside the evidence archives, passes on the whole tree. All gates passed on `7d8111b`: release-validation 66/66 (run 35960703932), standard CI 20/20 (run 35960703465), documentation (run 35960703436). The registry-installed artifact passed 32/32 independent consumer assertions (import, render, PNG round-trip, standalone WebGL export, forward and reverse inverse problems, documented rejection paths). The evidence log records the full run history including the eliminated hypotheses |
...
- Exported fragment shaders declared `mediump`, which WebGL 1 guarantees at only
  ten mantissa bits. Headless Linux Firefox cannot create a WebGL context without
  a display, and the hosted macOS runners give it a software rasteriser that
  drops clipped triangles, so it is validated on Linux under Xvfb.
