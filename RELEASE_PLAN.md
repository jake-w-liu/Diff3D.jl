...
| R4 | Verify the supported Julia versions on Linux, macOS, and Windows; exercise browser export on Chromium, Firefox, and WebKit. Preserve numerical, resource-lifetime, concurrency, and example checks. Record the tested architectures and browser/GPU environments. | In progress | Full matrix exposed Windows formatting, checkout and report-path defects; repairs passed local formatter/export and checkout checks; actual Windows rerun remains. Browser sampler/location checks are recorded in the evidence log |
