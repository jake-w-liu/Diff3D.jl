...
const _API_TYPE_DOCS = Dict{Symbol,String}(
    ...
    :SkinnedMesh => "Mesh whose vertices are transformed by weighted skeleton bones.",
    ...
    :MeshBasicMaterial => "Unlit mesh material with optional maps, opacity, and side mode.",
    :MeshLambertMaterial => "Diffuse Lambert material for light-dependent matte shading.",
    :MeshPhongMaterial => "Phong material with diffuse and specular highlights.",
    :MeshStandardMaterial => "Metallic-roughness PBR-style material.",
    :MeshPhysicalMaterial => "Extended physical material with clearcoat, sheen, transmission, and related terms.",
    :MeshNormalMaterial => "Debug material visualizing interpolated normals as color.",
    :MeshToonMaterial => "Banded toon/diffuse material.",
    :MeshMatcapMaterial => "Matcap-style view-facing material approximation.",
    :MeshDepthMaterial => "Depth-visualization material.",
    ...
    :QuaternionKeyframeTrack => "Quaternion keyframe track using spherical interpolation.",
    ...
    :GLTFAsset => "Structured result from `load_gltf_asset`/`load_glb_asset` with scenes, nodes, animations, and cameras.",
...
const _API_FUNCTION_DOCS = Dict{Symbol,String}(
    ...
    :render_tiled! => "Render a scene in scanline/tile bands for large images or bounded memory.",
    :render_pooled! => "Render using a reusable `RenderCache` to reduce repeated allocations.",
...
const _GENERIC_DOCS = Dict{Symbol,String}(
    ...
    :apply_skinning => "Apply linear blend skinning to mesh vertices.",
