...
const _API_TYPE_DOCS = Dict{Symbol,String}(
    ...
    :MeshPhongMaterial => "Phong material with diffuse/specular highlights, optional specular/glossiness, normal, AO, and light maps, vertex colors, and local clipping planes.",
    ...
    :MeshToonMaterial => "Banded toon/diffuse material with optional albedo, alpha, normal, AO, light, emissive, and gradient-map sampling.",
    :MeshMatcapMaterial => "Matcap-style view-facing material approximation with optional albedo, alpha, normal, matcap, and wireframe fields.",
    :MeshDepthMaterial => "Depth-visualization material with basic/RGBA/RGB/RG packing modes plus map alpha, alpha-map, and wireframe fields.",
...
const _API_FUNCTION_DOCS = Dict{Symbol,String}(
    ...
    :load_jpeg => "Decode JPEG/JPG data into a texture-compatible image representation.",
    :load_image => "Decode a PNG or JPEG/JPG image by inspecting its file bytes.",
    :load_rgbe => "Decode Radiance RGBE/HDR image data into a linear floating-point image.",
    :load_hdr => "Alias for `load_rgbe` for `.hdr` environment-map files.",
    :audio_duration => "Return the duration in seconds for decoded audio buffer data.",
    :load_wav => "Decode RIFF/WAVE PCM or IEEE-float audio into an audio buffer.",
    :load_audio => "Decode supported audio files into an audio buffer.",
    :load_font => "Load a three.js typeface JSON font.",
    :font_glyph_shapes => "Flatten one loaded font glyph into point loops.",
    :font_text_shapes => "Flatten loaded font glyph outlines for text into point loops.",
    :font_kerning => "Return a parsed typeface kerning adjustment for a glyph pair.",
    ...
    :equirectangular_to_cubemap => "Convert an equirectangular environment texture into a cubemap.",
...
const _GENERIC_DOCS = Dict{Symbol,String}(
    ...
    :TextureLoader => "Load a PNG or JPEG/JPG image texture from disk.",
    :RGBELoader => "Load a Radiance RGBE/HDR image as a linear texture.",
    :AudioBufferData => "Decoded audio buffer data with samples stored as frames by channels.",
    :AudioLoader => "Load a RIFF/WAVE audio buffer from disk.",
    :FontCommand => "Parsed command from a three.js typeface glyph outline.",
    :FontGlyph => "Typeface glyph metadata and parsed outline commands.",
    :FontData => "Decoded three.js typeface JSON font data.",
    :FontLoader => "Load a three.js typeface JSON font.",
    :font_kerning => "Return a parsed typeface kerning adjustment for a glyph pair.",
    :SVGStyle => "Inherited SVG fill/stroke presentation data for one parsed path.",
    :SVGPath => "Parsed SVG primitive or path subpath.",
    :SVGDocument => "Decoded SVG document with common path, shape, line, bounded URL/basic-shape clipped, and vector-masked primitives converted to paths.",
    :SVGLoader => "Load common SVG paths, primitives, bounded URL/basic-shape clip-paths, and vector URL masks into an SVG document.",
